...
export function Navbar() {
  ...
  function handleSearchKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setSelectedSuggestionIdx((i) => Math.min(i + 1, items.length - 1)) }
    else if (e.key === "ArrowUp") { e.preventDefault(); setSelectedSuggestionIdx((i) => Math.max(i - 1, -1)) }
    else if (e.key === "Enter") {
      e.preventDefault()
      if (selectedSuggestionIdx >= 0 && items[selectedSuggestionIdx]) handleSearch(items[selectedSuggestionIdx].text)
      ...
      <header className="sticky top-0 z-50 bg-background border-b border-border shadow-sm">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-6 h-16 flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center shrink-0">
            <span className="text-2xl font-black tracking-tighter text-brand">AJIO</span>
          </Link>

          <nav className="hidden lg:flex items-center gap-1 ml-2">
            <Link
              to="/products"
              className="px-3 py-2 text-sm font-medium rounded-md text-foreground/80 hover:text-foreground hover:bg-accent transition-colors"
            >
              All Products
            </Link>
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    activeMenu === cat.slug
                      ? "text-brand bg-brand-light dark:bg-accent"
                ...
                {activeMenu === cat.slug && (
                  <div
                    onMouseEnter={() => handleMenuEnter(cat.slug)}
                    onMouseLeave={handleMenuLeave}
                    className="absolute top-full left-0 mt-0 bg-popover border border-border rounded-b-xl shadow-xl z-50 min-w-[200px] p-4 animate-in fade-in-0 zoom-in-95 duration-150"
                  >
                    <Link
                      to={`/products?category=${cat.slug}`}
          ...
          <div ref={searchRef} className="flex-1 max-w-xl mx-auto relative">
            <div className="relative flex items-center">
              ...
              <input
                ...
                value={searchQuery}
                onChange={(e) => { setSearchQuery(e.target.value); setShowSuggestions(true); setSelectedSuggestionIdx(-1) }}
              ...
              />
              {searchQuery && (
                <button onClick={() => { setSearchQuery(""); setSuggestions([]) }} className="absolute right-3 text-muted-foreground hover:text-foreground">
                    ...
                    <ul>
                      {suggestions.map((s, i) => (
                        <li key={s.id ?? i}>
                          ...
                          >
                            {s.image && <img src={s.image} alt="" className="size-8 rounded object-cover shrink-0" />}
                            <span className="text-muted-foreground">{highlightMatch(s.text, searchQuery)}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
                    ...
                    {recentSearches.length > 0 && (
                      <div>
                        <div className="flex items-center justify-between px-4 pt-3 pb-1">
                          ...
                          <button onClick={clearRecentSearches} className="text-xs text-brand hover:underline">Clear</button>
                        </div>
                        {recentSearches.map((s, i) => (
                          <button key={i} onClick={() => handleSearch(s)} className="w-full text-left px-4 py-2 text-sm flex items-center gap-3 hover:bg-accent transition-colors">
